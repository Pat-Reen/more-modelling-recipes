...
The Indeed connector isn't that kind of server. It's installed through claude.ai's connector directory, which handled an OAuth handshake with Indeed the one time I connected it, in a browser, and holds the resulting credential on Anthropic's side from then on, tied to my account and to interactive sessions like this one. There's no export button, no bearer token to copy into a GitHub secret - not because the feature is missing, but because that credential was never issued as a portable thing in the first place. The OAuth flow that authorised it needs a browser and a login, which isn't something a cron job can do on its own.
