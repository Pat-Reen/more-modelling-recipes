...
That's a cleaner result than a general web search gives you - structured fields, a stable job ID, a canonical listing URL, no scraping a search results page and hoping the format holds. My first instinct was that the daily scan should use it instead of `web_search`. It can't, but not because MCP is somehow off-limits to a script - the Claude API has a documented, fully programmatic way to call a remote MCP server, and I want to be precise about what actually blocks this one.

The Claude API supports connecting directly to a remote MCP server from a single request, no interactive session or separate client library required:
...
That's a plain API call - the same shape `scan.py` already uses, just with an `mcp_servers` block and a matching `mcp_toolset` tool added. If a server takes a bearer token, there's even a field for it: `authorization_token` on the server definition. So the split isn't "MCP works in a script" versus "MCP doesn't" - it's what kind of credential the server needs. An MCP server that authenticates with a static secret - an API key, a personal access token, a service account credential you generated yourself - drops straight into `authorization_token` and a script can call it today, no session involved.

The Indeed connector isn't that kind of server, and it's worth being precise about why, because the chat case and the script case aren't doing the same thing underneath even though they look like the same connector. Indeed only issues OAuth client registrations - the credentials an app needs to even start a login flow - to companies approved through a sales-led partner process. Anthropic has that relationship; I don't, individually. When I clicked "connect" on the Indeed connector, I went through a login screen under *Anthropic's* registered app, granting that app permission to search Indeed on my behalf. The resulting token lives in Anthropic's connector infrastructure, scoped to my account, and is used only when Anthropic's backend is serving an authenticated Claude session for me - chat, Claude Code, wherever I'm logged in. There's no export button, because a connector is built to be used by Claude, for me, inside a Claude session - not as a general-purpose credential I can lift out and hand to a GitHub Actions job, which isn't a Claude session and isn't logged in as me.

Could I get my own independent, script-usable Indeed credential instead, bypassing Anthropic entirely? That's how a lot of "authorise once, automate forever" integrations work - one interactive consent, then a long-lived token your own code holds - so it's a fair question. It's blocked at the first step: to start that flow at all, Indeed would need to issue *me* an OAuth client registration, and that's the same partner-approval gate, not a self-serve signup for an individual. So the honest options for actually using this connector in the pipeline are an MCP server that wraps Indeed data with its own portable credentials (I don't know of one, and standing one up runs straight into the same access problem from a different angle), or replacing the plain script with a scheduled interactive Claude session that has the connector attached and can push its own commits. I looked hard at the second option and decided against it for now - it turns a five-second API call into a full agent run on a schedule, with the cost and unpredictability that comes with it, for a tool that's currently working fine on plain web search. It's the right call for a case where the connector genuinely can't be replicated by search - internal ticketing systems, a paid data source with no public equivalent - and the value of that data justifies an agent session over a script. Worth knowing the door is open, even if I'm not walking through it today.
