...
One implementation detail worth noting: Grok returns JPEG even when PNG is implied by the prompt. If you trust the file extension, Claude throws a media type mismatch error and the whole pipeline stalls. The fix is to detect the actual format from the first two bytes of the file — magic bytes — rather than the extension. JPEG files start with `FF D8`; PNG files start with `89 50 4E 47`. It's a small thing, but it's the kind of edge case that's invisible until it isn't, and when it breaks it breaks silently in a confusing place.
