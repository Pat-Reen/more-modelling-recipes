...
# use system Python (no conda required)
use_python(Sys.which("python"), required = TRUE)

# if packages are not already installed, run once in a terminal:
# pip install pandas numpy scipy matplotlib seaborn scikit-learn linearmodels tabulate rpy2
...
# Define date range
date_range = pd.date_range(start='2019-01-01', end='2024-06-30', freq='ME', normalize=True)
...
# pandas (2.x with pyarrow) returns string columns as ArrowStringArray,
# which reticulate doesn't auto-convert to R character; force them here
df$age_group        <- as.character(df$age_group)
df$occupation       <- as.character(df$occupation)
df$product          <- as.character(df$product)
df$change_direction <- as.character(df$change_direction)
